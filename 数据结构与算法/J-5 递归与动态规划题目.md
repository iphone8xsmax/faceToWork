...
func isSubsequence(s string, t string) bool {
    ...
    for i := 0; i < n; i++{
    	...
    	//匹配到字符之后，将add向后移，继续匹配下一个字符，即f[i+1][j]
...
![image-20201028171553759](J-5 递归与动态规划题目.assets/image-20201028171553759.png)
...
func lengthOfLIS(nums []int) int {
    ...
    res := []int{nums[0]}
    for i := 1; i < length; i++{
        if nums[i] > res[len(res) - 1]{ //如果当前数比res中末尾数大，直接加入
            res = append(res, nums[i])
        }else{ //否则，使用二分法查找应该插入的位置
            l, r := 0, len(res) - 1
            for l <= r{
                m := l + (r - l) >> 1
                if nums[i] > res[m]{
...
在上面的代码中不只求**解出了上升子序列的长度**，也求出了具体的上升子序列。
...
[413. Arithmetic Slices (Medium)](https://leetcode-cn.com/problems/arithmetic-slices/submissions/)
...
[343. Integer Break (Medim)](https://leetcode-cn.com/problems/integer-break/)
...
func numSquares(n int) int {
    ...
    for i:=2;i<=n;i++{
        for j:=0;j*j<=i;j++{
            dp[i]=Min(dp[i],dp[i-j*j]+1)        //递推关系式，如果这个数字存在多种这样的情况：该数组可以被多个数的
        }                                       //平方和相加，则取最小的一种情况 
    }
    return dp[n]
}
...
[646. Maximum Length of Pair Chain (Medium)](https://leetcode-cn.com/problems/maximum-length-of-pair-chain/)
...
func findMaxForm(strs []string, m int, n int) int {
    ...
    // dp[i][j]表示最多有 i 个 0 和 j 个 1 时候的最大值
    dp := make([][]int, m + 1)
    for i := 0; i < m + 1; i++{
        dp[i] = make([]int, n + 1)
    }
    ...
    for _, str := range strs{
        ...
        // 只有当剩余的 0 1 数量大于当前子串的数量，才可以考虑是否使用
        for i := m; i >= zeros; i--{
            for j := n; j >= ones; j--{
...
func maxProfit(prices []int) int {
    ...
    dp := make([][3]int, n + 1)
    dp[0][0] = -prices[0] //第一天如果买入

    for i := 1; i < n; i++{
        dp[i][0] = max(dp[i - 1][0], dp[i - 1][2] - prices[i]) //当前手上持有股票是前一天即持有，或者是当天购入
        dp[i][1] = dp[i - 1][0] + prices[i] //当天处于冷静期，说明前一天卖出
        dp[i][2] =  max(dp[i - 1][1], dp[i - 1][2]) //当天不在冷静期，说明前一天在冷静期或者前一天也未进行操作
    }
    return max(dp[n - 1][1], dp[n - 1][2]) //最后一天肯定是手上不持有股票的
}
...
func maxProfit(prices []int, fee int) int {
    ...
    //dp[i][0]表示第 i 天结束手头没有股票的最大收益
    //dp[i][1]表示第 i 天结束手头有股票的最大收益
    dp := make([][2]int, n) 
    dp[0][1] = -prices[0]
...
给定数组 arr, arr 中所有的值都为正数且不重复。每个值代表一种面值的货币，每种面值的货币可以使用**任意张**，再给定一个整数 aim 代表要找的钱数，求组成 aim 的最少货币数。
